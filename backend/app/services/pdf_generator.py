...
class PDFReportGenerator:
    ...
    def generate_report(
        ...
        story.append(Paragraph(
            "ОТЧЕТ ПО ВОЗРАСТНОМУ РЕЙТИНГУ" if self.language == "ru" else "AGE RATING REPORT",
    ...
    def _create_header_section(self, script: Script) -> List:
        ...
        header_data = [
            ...
            ["Дата создания:", script.created_at.strftime("%d.%m.%Y %H:%M") if script.created_at else "—"],
    ...
    def _create_scores_chart(self, scores: Dict[str, float]) -> Optional[Image]:
        ...
        colors_list = ['#ef4444' if v > 60 else '#f59e0b' if v > 30 else '#10b981' for v in values]
    ...
    def _create_scenes_section(self, scenes: List[Scene]) -> List:
        ...
        problematic_scenes = sorted(
            [s for s in scenes if max(
                s.violence, s.gore, s.sex_act, s.nudity,
                s.profanity, s.drugs, s.child_risk
            ) > 0.3],
            key=lambda s: max(s.violence, s.gore, s.sex_act, s.nudity, s.profanity, s.drugs, s.child_risk),
            reverse=True
        )[:10]

        if not problematic_scenes:
            elements.append(Paragraph("Проблемные сцены не обнаружены", self.styles['Normal']))
            return elements

        for scene in problematic_scenes:
            scene_elements = []

            scene_elements.append(Paragraph(
                f"<b>Сцена {scene.scene_id}: {scene.heading}</b>",
                self.styles['SubSection']
            ))

            issues = []
            for cat in ['violence', 'gore', 'sex_act', 'nudity', 'profanity', 'drugs', 'child_risk']:
    ...
    def _create_gaps_section(self, rating_gaps: List[Dict[str, Any]]) -> List:
        ...
        elements.append(Paragraph("Необходимые изменения", self.styles['SectionHeader']))
        ...
        gaps_table = Table(gaps_data, colWidths=[2*inch, 0.8*inch, 0.8*inch, 0.8*inch, 1*inch])
