...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_line_detections_id'), 'line_detections', ['id'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['script_id'], ['scripts.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['detection_id'], ['line_detections.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_user_corrections_id'), 'user_corrections', ['id'], unique=False)
