...
def analyze_whatif(request_data: dict) -> dict:
    ...
    response = requests.post(
        f"{ML_SERVICE_URL}/what_if_advanced",
        json=request_data
    )
    response.raise_for_status()
    return response.json()
...
def print_results(original: dict, whatif_result: dict):
    ...
    print(f"  Rating Changed: {'✅ YES' if whatif_result['rating_changed'] else '❌ NO'}")
    ...
    for etype, entities in entities_by_type.items():
        ...
        for entity in entities[:5]:
            print(f"    • {entity['name']} ({entity['mentions']} mentions in scenes {entity['scenes'][:3]}...)")
    ...
    print("  Scene Types:")
    for stype, count in sorted(scene_type_counts.items(), key=lambda x: x[1], reverse=True):
        print(f"    • {stype}: {count}")
