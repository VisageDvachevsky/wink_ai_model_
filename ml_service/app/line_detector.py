...
def extract_character_name(line: str, previous_line: str = "") -> str | None:
    ...
    if previous_line and re.match(
        r"^[A-ZА-ЯЁ][A-ZА-ЯЁ\s\.\-]{1,}$", previous_line.strip()
    ):
...
class LineDetector:
    def __init__(self):
        ...
        logger.info(
            f"LineDetector initialized with {len(CATEGORY_PATTERNS)} categories"
        )

    def detect_lines(self, text: str, context_size: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        for line_idx, line in enumerate(lines):
            ...
            for category, patterns in self.compiled_patterns.items():
                ...
                if matches:
                    ...
                    parents_guide_severity = get_parents_guide_severity(
                        severity, category
                    )
    ...
    def _calculate_severity(self, category: str, match_count: int, line: str) -> float:
        ...
        if any(
            word in line.lower()
            for word in ["graphic", "explicit", "extreme", "brutal"]
        ):
            severity += 0.2
